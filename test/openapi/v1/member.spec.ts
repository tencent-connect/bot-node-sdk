...
describe('member测试', () => {
  test('【 memberAddRole方法 】=== 增加、移除频道身份组成员', async () => {
    ...
    const addRoleRes = await client.memberApi.memberAddRole(guildID, ROLE_GOOLDEN_ADMIN_ID, userID, channelInfo);
    ...
    // 3.删除金牌管理员权限
    const removeRoleRes = await client.memberApi.memberDeleteRole(guildID, ROLE_GOOLDEN_ADMIN_ID, userID, channelInfo);
